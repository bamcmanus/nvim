-- Clipboard support for tmux and SSH sessions. Copies are emitted as OSC 52
-- so terminals can receive them, while local Wayland clipboard access remains
...
function M.setup()
  ...
  local in_herdr = vim.env.HERDR_PANE_ID ~= nil
    or (vim.fn.isdirectory("/proc") == 1 and ancestor_process_named("herdr"))
  ...
  local osc52_enabled = vim.g.remote_clipboard_osc52
  if osc52_enabled == nil then
    -- Preserve the old setting name for existing local configurations.
    osc52_enabled = vim.g.omarchy_remote_clipboard_osc52
  end
